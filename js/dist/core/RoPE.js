 ...
 */
export function initRoPECache(maxSeqLen, headDim, theta = ROPE_THETA) {
 ...
 */
export function applyRoPE(x, cache, seqLen, nHeads, dim) {
 ...
 * @param nHeads - Number of attention heads
 */
export function applyRoPESingle(q, cache, pos, nHeads) {
    const { cos, sin, headDim } = cache;
    const halfDim = headDim / 2;
    for (let h = 0; h < nHeads; h++) {
        for (let i = 0; i < halfDim; i++) {
 ...
 * @param maxSeqLen - Maximum sequence length
 */
export function applyRoPEToCache(k, cache, seqLen, nKvHeads, maxSeqLen) {
    const { cos, sin, headDim } = cache;
    const halfDim = headDim / 2;
    for (let t = 0; t < seqLen; t++) {
 ...
 * @param maxSeqLen - Maximum sequence length
 */
export function applyRoPEToSinglePos(k, cache, pos, nKvHeads, maxSeqLen) {
    const { cos, sin, headDim } = cache;
    const halfDim = headDim / 2;
    for (let kh = 0; kh < nKvHeads; kh++) {
